    ...
    return (
      <div className={styles.dataContainer}>
        ...
        <Row className={styles.row}>
          ...
          {/* 话务指标区域 */}
          <Col span={this.state.expand ? 24 : 8} className={styles.phoneCol} style={{position: !this.state.expand ? 'absolute' : '', right: !this.state.expand ? '0' : ''}}>
              ...
              <Row>
                ...
                <Col span={8} style={{ display: this.state.expand ? 'block' : 'none' }}>
                  <div className={styles.perAccumulate}>
                    <Indicator title='人均实时平均通时(上一时段)' width='80%' style={{height: 38}} data={this.state.realtimeAvgTalktime} />
                  </div>
                  <div className={styles.perAccumulate}>
                    <Indicator title='人均实时有效通时(上一时段)' width='80%' style={{height: 38}} data={this.state.realtimeEffecTalktime} />
                  </div>
                </Col>
                <Col span={8} className={styles.ColTalk} style={{ display: this.state.expand ? 'block' : 'none' }}>
                  <div className={styles.perAccumulate}>
                    <Indicator title='人均当日累计通时' width='80%' style={{height: 38}} data={this.state.todayTotalTalktime} />
                  </div>
                  <div className={styles.perAccumulate}>
                    <Indicator title='人均当日累计平均通时' width='80%' style={{height: 38}} data={this.state.todayTotalAvgTalktime} />
                  </div>
                </Col>
                <Col span={8} className={styles.ColTalknum} style={{ display: this.state.expand ? 'block' : 'none' }}>
                  <div className={styles.perAccumulate}>
                    <Indicator title='人均实时有效通次(上一时段)' width='80%' style={{height: 38}} data={this.state.realtimeEffecTalknum} />
                  </div>
                  <div className={styles.perAccumulate}>
                    <Indicator title='人均当日累计通次' width='80%' style={{height: 38}} data={this.state.todayTotalTalknum} />
                  </div>
                </Col>
              </Row>
            </div>
